...
/* This code detects Vertical Edges from greyscale images         */
/******************************************************************/
#include <stdio.h>
#include <math.h>
#include <X11/Xlib.h>
...
unsigned char proc_img[DIM][DIM];
{
	...
	int deriv_y_mask[3][3] = {{1,1,1},{0,0,0},{-1,-1,-1}};	//Prewitts Template
	int grad_y[size[0]][size[1]];	//Holds sum of convolution
	...
	for(x = 0; x<height; x++)
	{
		for(y = 0; y<width; y++)
		{
			...
			{	
				...
				for(i=-1; i<2; i++){
					for(j=-1; j<2; j++){
						y_sum+=image[y+i][x+j] * deriv_y_mask[i+1][j+1] / 3;//Convolution formula (Divide by 3 to keep values in range)
					}
				}
				grad_y[y][x] = y_sum;
				
				hyp = hypot(grad_y[y][x], 0);//Calculate the magnitude
				if(hyp>255)
